...
public class MessageBoardController {
	...
	public String getPost(@PathVariable Integer memberid,@PathVariable Integer postid 
		...
		for (Post post : posts) {
            ...
            String formattedPostTime = postTimeFromDb.replace(" ", "T");// 假設要將空格替換為 T
            post.setPosttime(formattedPostTime); 
            
            //將抓到的memberphoto轉為base64
            byte[] postMemberPhotoFromDb = post.getMember().getMemberphoto();
            post.getMember().setMemberphoto(postMemberPhotoFromDb);
            //將抓到的postphoto轉為base64
            List<PostPhoto> postphotos = post.getPostphoto();
            for(PostPhoto photo : postphotos) {
	            byte[] postPhotoFromDb = photo.getPostedphoto();
	            photo.setPostedphoto(postPhotoFromDb);
            }
        }
