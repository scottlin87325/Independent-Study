document.addEventListener('DOMContentLoaded', function () {
    ...
    const API = {
        ...
        updateLike: (id) => `/api/posts/${id}/like`,
        addComment: (id) => `/api/posts/${id}/comments`,
        deleteComment: (postId, commentId) => `/api/posts/${postId}/comments/${commentId}`,
        uploadPhoto: '/api/posts/upload-photo'
    };
    ...
    function initImageCropper(file) {
        ...
        if (confirmCropBtn) {
            confirmCropBtn.onclick = async () => {
                ...
                try {
                    const cropData = cropperInstance.getData(true); // true 參數獲取實際像素值
    ...
    function bindPostEvents(postElement, post) {
        // Bind all post events (like, comment, carousel, etc.)
